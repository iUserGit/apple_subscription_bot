...
def verify_certificate_chain(cert_chain_base64, root_ca_cert):
    ...
    try:
        ...
        if isinstance(issuer_key_1, RSAPublicKey):
            issuer_key_1.verify(
            ...
            issuer_key_1.verify(
                intermediate_cert.signature,
                intermediate_cert.tbs_certificate_bytes,
                ECDSA(intermediate_cert.signature_hash_algorithm),
            )
        else:
        ...
        if isinstance(issuer_key_2, RSAPublicKey):
            issuer_key_2.verify(
                leaf_cert.signature,
            ...
            issuer_key_2.verify(
                leaf_cert.signature,
                leaf_cert.tbs_certificate_bytes,
                ECDSA(leaf_cert.signature_hash_algorithm),
            )
        else:
...
def handle_app_store_notification():
    ...
    try:
        ...
        decoded_data = jws.deserialize_compact(signed_payload.encode('utf-8'), public_key)
        payload_bytes = decoded_data['payload']
        ...
        if notification_data and notification_data.get('signedTransactionInfo'):
            try:
                ...
                # Для проверки вложенного JWS, используем тот же ключ и инстанс jws
                decoded_transaction_data = jws.deserialize_compact(signed_transaction_info.encode('utf-8'), public_key)
                transaction_payload_bytes = decoded_transaction_data['payload']
